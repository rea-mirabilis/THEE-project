...
return(c(N_RR_new, N_Rr_new, N_rr_new))
}
```
...
simulate_one_gen_diploid <- function(N_RR, N_Rr, N_rr, decay_rate, sel_coeffs, mut_rate) {
  #set selective coeffs of different genotypes
  ...
  mut_R_to_r <- rpois(1, offsp_RR * mut_rate)  # RR individuals mutate into Rr
  mut_r_to_R <- rpois(1, offsp_rr * mut_rate)  # rr individuals mutate into Rr

  # Update population sizes based on mutations and offspring survival
  N_RR_new <- max(offsp_RR - mut_R_to_r, 0)
  N_Rr_new <- max(offsp_Rr + mut_R_to_r + mut_r_to_R, 0)
  N_rr_new <- max(offsp_rr - mut_r_to_R, 0)
  
  mating_function(N_RR_new, N_Rr_new, N_rr_new)
}
...
max_gen <- 100
# create your simulation data
output <- simulate_pop_diploid(N_RR = 100, N_Rr=100, N_rr=100, decay_rate=0.1, sel_coeffs=0.2, mut_rate=0.1,max_gen)
# show the last few lines of the data table
print(tail(output))
# plot the output - note that the plot range and the x axis have to be adjusted based on the length of the table
# determine x axis range 
...
# mating algorithm for tetraploid
...
#test the function
print(simulate_one_gen_tetraploid(Init_RRRR, Init_RRRr, Init_RRrr, Init_Rrrr, Init_rrrr, 0.1, sel_coeffs, 0.01))
```

# simulating population tetraploid(not started)
